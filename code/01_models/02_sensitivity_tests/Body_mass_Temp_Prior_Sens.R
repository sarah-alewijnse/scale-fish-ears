#### MCMC GLMMs - C_resp - Body mass & Temperature ####
...
Z_BM <- Z.score(C_resp_tidy$Body_Mass, "Body_mass")
Z_log_BM <- Z.score(C_resp_tidy$log_BM, "log_BM")
Z_temp <- Z.score(C_resp_tidy$mean_Temp, "Temp")

C_resp_tidy <- cbind(C_resp_tidy, Z_BM, Z_log_BM, Z_temp)

glimpse(C_resp_tidy)

# Histogram

ggplot(C_resp_tidy, aes(x = Z_Body_mass)) +
  geom_histogram()

ggplot(C_resp_tidy, aes(x = Z_log_BM)) +
  geom_histogram()

ggplot(C_resp_tidy, aes(x = Z_Temp)) +
  geom_histogram()
...
check <- name.check(phy = fish_tree,
                    data = C_resp_tidy,
                    data.names = C_resp_tidy$sciname)
check$tree_not_data
check$data_not_tree
...
mod_BM_temp_sens <- MCMCglmm(mean_C_resp ~ Z_log_BM + Z_Temp, 
                            random = ~ sciname + # phylogenetic effects
                              Species_ID + # intraspecific effects
                              n + # number of samples per data point
                              Source + # otolith source
...
saveRDS(mod_BM_temp_sens, "Outputs/07_C_resp_Models/MCMC_GLMM/01_Combined/BM_temp_sensitivity.rds")
